...
export default function Inventory({ products, onAdd, onUpdate, onDelete, canEdit, canDelete, settings, categories: catProp }) {
    ...
    <div className="bs-inv">
      ...
      <div className="bs-inv-bar">
        <div className="bs-inv-bar-l">
          <input className="bs-inp" style={{width:'240px'}} placeholder="🔍 Search name / code / barcode…" value={search} onChange={e=>setSearch(e.target.value)}/>
      ...
      <div className="bs-tbl-wrap">
        <table className="bs-tbl">
          <thead>
            <tr>
              <th>Code</th><th>Product (Brand · Name · Size)</th><th>Photo</th>
              <th>Category</th><th>Barcode</th><th>Sale Price</th><th>Cost</th>
              <th>Margin</th><th>Stock</th><th>Unit</th><th>Min</th><th>Last Receive</th>
            ...
            {filtered.map(p => {
              ...
              return (
                <tr key={p.id} className={p.stock===0?'tr-out':p.stock<=p.minStock?'tr-low':''}>
                  ...
                  <td>{p.photoUrl ? <img src={p.photoUrl} alt="" style={{width:32,height:32,objectFit:'cover',borderRadius:4}}/> : <span className="bs-muted">—</span>}</td>
      ...
      {(modal?.type==='add'||modal?.type==='edit') && (
        <div className="bs-overlay" onClick={e=>e.target===e.currentTarget&&setModal(null)}>
          <ProductForm product={modal.data} onSave={handleSave} onClose={()=>setModal(null)} fmt={fmt} existingCompanies={getCompanies(products)} existingCodes={products.map(p=>p.productCode).filter(Boolean)} categories={CATEGORIES}/>
        </div>
      )}
      {modal?.type==='bulk' && (
        <div className="bs-overlay" onClick={e=>e.target===e.currentTarget&&setModal(null)}>
          <BulkImport onAdd={onAdd} onUpdate={onUpdate} products={products} onClose={()=>setModal(null)} categories={CATEGORIES}/>
        </div>
      )}
      {confirm && (
...
function ProductForm({ product, onSave, onClose, fmt, existingCompanies, existingCodes, categories: catProp }) {
  ...
  const handlePhoto = e => {
    ...
    if (!file) return;
    const MAX_PX = 400; // max width or height
    const QUALITY = 0.75;
    const img = new Image();
      ...
      <form className="bs-form" onSubmit={submit}>
        ...
        <div className="bs-fg">
          ...
          <div style={{display:'flex',gap:'10px',alignItems:'center'}}>
            {f.photoUrl && <img src={f.photoUrl} alt="" style={{width:48,height:48,objectFit:'cover',borderRadius:6,border:'1px solid #2a3a5c'}}/>}
            <label className="bs-sec" style={{padding:'7px 14px',cursor:'pointer',display:'inline-block'}}>
              📷 Choose Photo
